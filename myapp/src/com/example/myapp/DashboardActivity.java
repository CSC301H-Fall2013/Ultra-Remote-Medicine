	...
	public void onCreate(Bundle savedInstanceState) {
		...
		searchButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View view) {
				...
				Thread thread = new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							jsonCurSearchList = communicate(urlString,
									jsonString);
				...
				int timer = 0;
				while (jsonCurSearchList == null && timer < 300) {
					try {
						Thread.sleep(200);
				...
				if (timer >= 300
						|| jsonCurSearchList.optString("success").equals(
								"false")) {
					String msgString = (timer < 300 ? "Fail to search the key"
							: "Server time out");
